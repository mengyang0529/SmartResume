      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-[rgba(0,0,0,0.1)]">
        <div className="flex items-center h-[55px] px-4 md:px-6">
          ...
          <Link to="/" className="flex items-center gap-2.5 group shrink-0">
            <span className="text-[15px] font-semibold text-[rgba(0,0,0,0.95)] leading-none">
              Smart Resume
            </span>
          </Link>
        </div>
          ...
          <>
            ...
            >
              <div className="h-[55px] flex items-center justify-between px-6 border-b border-[rgba(0,0,0,0.05)] bg-white">
                <span className="text-sm font-bold uppercase tracking-wider text-warm-400">
                  Navigation
                </span>
                <button
                  onClick={() => setIsSidebarOpen(false)}
              ...
              <nav className="flex-1 py-6 px-3 space-y-1">
                {navItems.map(item => (
                  ...
                  >
                    <span
                      className={`text-base ${isActive(item.path) ? 'text-[#0075de]' : 'text-warm-400 group-hover:text-warm-600'}`}
                    >
                      {item.icon}
                    </span>
