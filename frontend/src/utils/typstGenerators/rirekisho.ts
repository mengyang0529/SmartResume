...
export function generateRirekishoTypst(data: ResumeData, _settings: TemplateSettings, skillsBlocks?: RichTextBlock[]): string {
  ...
  if (personal.furiganaFirstName) authorEntries.push(`    furigana-first: "${escapeTypstString(personal.furiganaFirstName)}",`)
  if (personal.furiganaLastName) authorEntries.push(`    furigana-last: "${escapeTypstString(personal.furiganaLastName)}",`)
...
#table(
  ...
  [#align(center)[*氏名*]],
  ${(personal.furiganaLastName || personal.furiganaFirstName) ? `[#align(left)[
    #grid(
      columns: (auto, auto),
      row-gutter: 2pt,
