...
import { escapeTypstContent, escapeTypstString, dataUrlToTypstBytes, getAccentColor } from './shared'
...
function renderBlocksAsEntries(blocks: RichTextBlock[], template?: string, accentColor?: string): string {
  ...
  description: [${currentEntry.description || ''}],
      ...
      if (currentEntry.items.length > 0) {
        typst += `#resume-item[\n`
        currentEntry.items.forEach(item => { typst += `  - ${item}\n` })
        typst += `]\n\n`
      }
    }
    currentEntry = null
...
export function generateAwesomeCvTypst(data: ResumeData, settings: TemplateSettings, skillsBlocks?: RichTextBlock[]): string {
